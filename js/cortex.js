...
class CortexMesh extends THREE.Object3D {
  constructor(layers) {
    super();
    //reference saved file on desktop to ensure nothing typed here results in error
...
//shortern this code for the website Publish - Rahul :V
    var uniforms = {
		  color:   {
  ...
  updateProximalConnections() {
    ...
    for (var k=1; k<this.numLayers; ++k) {
      var n = 0;
      //nodes value here increase based on iter
      var iPos = this.nodePos[k];
      for (var i=0, i0=0, i2=2; i<N; ++i, i0+=3, i2+=3) {
        var iData = this.nodeData[k][i];
        var nearest = this.findNearestNodes(iPos[i0], iPos[i2],
  ...
  updateNodePos() {
    ...
    for (var k=0; k<this.numLayers; ++k) {
	    for ( var i=0, i0=0, i1=1, i2=2; i<N; ++i, i0+=3, i1+=3, i2+=3 ) {
          ...
          iData.vel.z *= -1;
      }//updaate based values with the given atribute
  ...
  updateNodeStates() {
    ...
    for (var k=1; k<this.numLayers; ++k) {
        ...
        if (iData.activated) {
          //Node being activated; react, react, react
          sizes[i] *= 0.95;
          color[i0] *= 0.95;
          color[i1] *= 0.95;
          color[i2] *= 0.95;
